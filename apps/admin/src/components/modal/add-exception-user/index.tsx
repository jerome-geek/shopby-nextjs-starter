...
const AddExceptionUserModal = ({ ...props }: DefaultModalLayoutProps) => {
    ...
    const methods = useForm<CreateExceptionSchemaType>({
    ...
    const { createException: createExceptionMutation } = useLimitMutation();

    const onSubmit = async (data: CreateExceptionSchemaType) => {
        try {
            ...
            const results = await Promise.allSettled(
                uniqueMemberNos.map((memberNo) => {
                    ...
                    return createExceptionMutation.mutateAsync({
                        memberNo,
    ...
    return (
        <ModalLayout
            {...props}
            title='예외 계정 추가'
            subtitle='레시피 생성 제한이 적용되지 않을 계정을 등록합니다.'
            footer={
                <>
                    <button
                        type='button'
