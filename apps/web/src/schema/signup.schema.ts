...
const signupDuplicateCheckMemberIdSchema = z
    ...
    .regex(
        regEx.memberIdNew,
        '아이디는 5자 이상의 영문, 숫자만 사용 가능합니다.',
    );

const signupDuplicateCheckEmailSchema = z
    .string('이메일을 입력해 주세요.')
    .min(1, '이메일을 입력해 주세요.')
    .regex(regEx.email, '이메일 형식이 올바르지 않습니다.');
...
const signupFormSchema = z
    .object({
        ...
        //     ? z.string().optional()
        //     : signupDuplicateCheckMemberIdSchema,
        memberId: signupDuplicateCheckMemberIdSchema,
        password: z
            .string('비밀번호를 입력해 주세요.')
            .nonempty('비밀번호를 입력해 주세요.')
            .min(8, '비밀번호는 최소 8자 이상이어야 합니다')
        ...
        memberName: isGlobalMall
            ? z.string().optional()
            : z.string('이름을 입력해주세요.').nonempty('이름을 입력해주세요.'),
        lastName: isGlobalMall
            ? z.string('성을 입력해 주세요.').nonempty('성을 입력해 주세요.')
            : z.string().optional(),
        firstName: isGlobalMall
            ? z
                  .string('이름을 입력해 주세요.')
